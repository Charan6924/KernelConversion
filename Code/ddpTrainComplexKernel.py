...
@torch.no_grad()
def validate(model, image_loader, lambda_recon, device):
    model.eval()
...
def plot_epoch_results(plot_data, epoch, out_dir):
    ...
    # (B, 512, 512) → take first batch item → (512, 512)
    ...
    axes[1, 2].plot(r_s2sh[mid], label='GT s→sh', color='tomato',    linewidth=1.5, linestyle='--')
    axes[1, 2].plot(r_sh2s[mid], label='GT sh→s', color='steelblue', linewidth=1.5, linestyle='--')
...
def main():
    ...
    out_dir  = Path("training_output_filter")
