...
def train_one_epoch(model, image_loader, mtf_loader, optimizer, scaler, l1_loss, alpha, device, epoch, is_main):
    model.train()
    ...
    for i, (I_smooth_1, I_sharp_1, I_smooth_2, I_sharp_2) in enumerate(loader):
        ...
        if i == 0 and is_main:
            print("I_gen_sharp shape:", I_gen_sharp.shape)      
...
def main():
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        train_stats, plot_data = train_one_epoch(
            ...
            optimizer, scaler, l1_loss, cfg.alpha, device, epoch=ep, is_main=is_main
        )
        val_stats = validate(
